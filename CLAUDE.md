...
Android + iOS. Каталог с тех пор вырос до 408 видов, подборок по странам 45,
...
переведены, подборки готовы у двенадцати стран из пятнадцати (`BE`, `CH`, `DK`,
`GR`, `IE`, `IS`, `LU`, `ME`, `MK`, `NL`, `NO`, `PT`), у остальных трёх (`AL`,
`BA`, `CY`) ждут исследования (`docs/research/europe-15/`, Фазы 1–2 плана). До
него подборки этих стран не существуют. На языке `bs` названия грибов
показываются латынью везде (файла `names/bs.json` ещё нет), на `sq` — почти
везде (`names/sq.json` есть, но в нём три названия: письменного видового
лексикона грибов на албанском не нашлось, подробности в
`docs/research/europe-15/results/README.md`). Это ожидаемое состояние, не
дефект.
