...
переведены, подборки готовы у пяти стран из пятнадцати (`CH`, `GR`, `IS`, `NO`,
`PT`), у остальных десяти (`AL`, `BA`, `BE`, `CY`, `DK`, `IE`, `LU`, `ME`, `MK`,
`NL`) ждут браузерного исследования (`docs/research/europe-15/`, Фазы 1–2
плана). До него подборки этих стран не существуют, а на языках `sq`, `bs`, `da`,
`mk`, `nl` названия грибов показываются латынью везде в приложении (файлов
`names/<lang>.json` для них ещё нет) — это ожидаемое состояние, не дефект.
