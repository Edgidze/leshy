...
переведены, подборки готовы у девяти стран из пятнадцати (`BE`, `CH`, `DK`,
`GR`, `IS`, `LU`, `NL`, `NO`, `PT`), у остальных шести (`AL`, `BA`, `CY`, `IE`,
`ME`, `MK`) ждут исследования (`docs/research/europe-15/`, Фазы 1–2 плана). До него
подборки этих стран не существуют, а на языках `sq`, `bs`, `mk` названия грибов
показываются латынью везде в приложении (файлов `names/<lang>.json` для них ещё
нет) — это ожидаемое состояние, не дефект.
