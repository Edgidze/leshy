...
- **MapLibre `OrnamentOptions` не умеет стекать орнаменты друг под другом** —
  ...
  `ui/map/CLAUDE.md`.
- **У любого растра обязан быть потолок ширины — доли экрана
  (`fillMaxWidth()`, `fillMaxWidth(0.92f)`) недостаточно.** Все картинки
  приложения маленькие: иллюстрации каталога — 242px по длинной стороне,
  служебные значки — 192px, снимок маршрута прогулки — 960×540. Доля экрана
  на телефоне даёт приемлемое растяжение, а на планшете — четырёхкратное и
  полную потерю чёткости (найдено на эмуляторе планшета 2026-09-24). Потолок
  каталожной картинки — `MUSHROOM_PHOTO_MAX_WIDTH` (`ui/components/
  MushroomTile.kt`), он же объясняет, откуда берётся число.
- **Ширина диалога — только через `Modifier.dialogWidth()`**
  (`ui/components/DialogWidth.kt`), не долей экрана. Все диалоги проекта идут с
  `usePlatformDefaultWidth = false`, а вместе с платформенной шириной
  отключается и потолок — включая СОБСТВЕННЫЙ потолок `AlertDialog`
  (Material 3 зажимает себя в 280–560dp, но модификатор вызывающего стоит в
  цепочке перед его `sizeIn`, и `fillMaxWidth(доля)` его отменяет). Порядок в
  самом `dialogWidth()` обратить нельзя, причина — в его KDoc.
- **Высота диалога — потолок, а не размер: `heightIn(max = maxHeight *
  DIALOG_HEIGHT_FRACTION)`, и `weight(1f, fill = false)` у прокручиваемой
  части внутри.** `fillMaxHeight(доля)` задаёт ТОЧНУЮ высоту, а обычный
  `weight(1f)` заставляет ребёнка её выбрать целиком — вдвоём они добивают
  пустотой любое короткое содержимое. Оба файла — `ui/components/
  DialogSizing.kt`.
...
**Второй продукт — российская редакция.** Решение и расчёты —
`.claude/plans/russia-edition.md`, план исполнения и дизайн —
`docs/russia-edition/` (начинать с `README.md`). Мировое приложение этим не
затрагивается; `applicationId` мирового «Лешего» — `leshy.mushrooms.map` — не
меняется никогда.

В сборке это **ось флейворов `edition`** (`world`/`russia`), отдельная от
`store` (`play`/`rustore`). Два правила, которые нужно знать независимо от
редактируемого файла:

- **Редакцию объявляет хост, а не `shared`.** Значение `Edition` приходит
  ровно одним путём — `initKoin(edition)` из `LeshyApplication.onCreate` и из
  `MainViewController(edition:)`; внутри `shared` оно нигде не вычисляется,
  `BuildConfig` для этого использовать нельзя (на iOS обе редакции стоят
  поверх одного `Shared.framework`). Подробности — `androidApp/CLAUDE.md`.
- **Различия редакций аддитивны: новые файлы, а не `if (edition == …)` внутри
  общих composable.** Проверяемый сигнал — счёт таких `if`: ноль хорошо,
  полтора десятка означает, что структуру пора пересматривать
  (`docs/russia-edition/README.md`, «Где посылка „один репозиторий" ломается»).
