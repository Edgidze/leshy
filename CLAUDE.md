...
Android + iOS. Каталог с тех пор вырос до 408 видов, подборок по странам 52,
...
переведены, подборки готовы у двенадцати стран из пятнадцати (`AL`, `BA`, `BE`,
`CH`, `CY`, `DK`, `GR`, `IS`, `LU`, `NL`, `NO`, `PT`), у остальных трёх (`IE`,
`ME`, `MK`) ждут исследования (`docs/research/europe-15/`, Фазы 1–2 плана). До
него подборки этих стран не существуют, а на языке `mk` названия грибов
показываются латынью везде в приложении (файла `names/mk.json` ещё нет) — это
ожидаемое состояние, не дефект. У `sq` названия есть, но их мало (14 на 52
позиции албанской подборки): албанской народной микономенклатуры почти не
существует в письменном виде, остальное штатно уходит на латинский фолбэк.
