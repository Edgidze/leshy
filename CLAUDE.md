...
2. **Локализация только через ключи** — `i18n/StringKey` + `Strings.kt`
   ...
   (названия 408 видов грибов и 45 стран, свои виды пользователя): это не
...
- **MapLibre `OrnamentOptions` не умеет стекать орнаменты друг под другом** —
  ...
  `ui/map/CLAUDE.md`.
- **Сетка, где ширина ячейки считается вручную, обязана считаться в целых
  пикселях, а не в `Dp`.** `FlowRow` округляет ширину ребёнка и отбивку
  отдельными `roundToPx()`, и при дробной плотности экрана сумма вылезает на
  пиксель за ширину ряда — ряд рассыпается в одну колонку. Живой пример с
  разбором — `FindTilesGrid` в `ui/components/StatsBlocks.kt`.
...
Android + iOS. Каталог с тех пор вырос до 408 видов, подборок по странам 55,
...
**Партия `europe-15` закончена:** девять новых языков интерфейса заведены и
переведены, подборки готовы у всех пятнадцати стран (`AL`, `BA`, `BE`, `CH`,
`CY`, `DK`, `GR`, `IE`, `IS`, `LU`, `ME`, `MK`, `NL`, `NO`, `PT`), исследования
по ним — `docs/research/europe-15/`. У `sq` названий мало (15 на 52 позиции
албанской подборки): письменной албанской микономенклатуры почти не существует,
остальное штатно уходит на латинский фолбэк — это ожидаемое состояние, не
дефект.
