...
enum class StringKey {
    ...
    // ── Ключи, существующие ради одной редакции ───────────────────────────────────────────────
    // Переведены только на `ru` и `en`, и это не недоделка: у российской редакции языков ровно
    // два (`EditionLanguages`), а сорок переводов строки, которую этот продукт никогда не
    // покажет, — шум. Остальные языки штатно уходят на английский фолбэк. Список ключей и
    // проверка полноты, которая их пропускает, — `EditionStrings.kt` и `StringsTest`.
    /** Имя продукта внутри интерфейса у российской редакции. */
    AppNameRussia,

    /** «Данные карты» у редакции, которая раздаёт тайлы со своего сервера. */
    AboutMapDataTextSelfHosted,
    // ──────────────────────────────────────────────────────────────────────────────────────────
