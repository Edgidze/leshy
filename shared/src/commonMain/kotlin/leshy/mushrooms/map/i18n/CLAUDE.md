...
## Переводы интерфейса завершены — 26 из 26 (Фазы 6–11)

`uiTranslations` полна: 24 файла `strings/Strings<Xx>.kt` по 251 ключу плюс
две exhaustive-`when`-ветки `ru`/`en`. Фолбэк на английский
(`?: englishStrings(key)`) с этого момента ничего не обслуживает — он
страховка на 27-й язык, добавленный раньше своего файла.

Единственная проверка полноты — `StringsTest
.everyTranslationMapIsCompleteAndNonBlank` (24 × 251 утверждение), потому что
компилятор для этих карт полноту не гарантирует (см. выше). Второй тест,
