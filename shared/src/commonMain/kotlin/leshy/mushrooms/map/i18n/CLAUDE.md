...
Формулировки подбирать под КОНТРОЛ, а не только под смысл: три варианта
темы живут в `SingleChoiceSegmentedButtonRow`, где на кнопку приходится
треть ширины экрана, — поэтому «Системное», а не «Как в системе», и
`Järjestelmä`, а не `Järjestelmän mukaan`.
