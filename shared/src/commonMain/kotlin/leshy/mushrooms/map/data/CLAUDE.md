...
- **`collections`** (v5) — подборки грибов по странам; `category_collections`
  ...
  **С v13 в этой же таблице живут пользовательские подборки**
  (`.claude/plans/user-collections.md`): `source`
  (`COUNTRY`/`USER`/`IMPORTED`) и `name` — литеральный ввод пользователя.
  `nameKey` у них — сгенерированный `usercoll_<ts>_<rand>`, идентификатор для
  слияния при импорте, а не текст; у служебной «Другие» он фиксированный
  (`collection_user_other`), поэтому «Другие» с разных устройств сливаются
  сами. `order`: страновые 0..N, пользовательские 1000, «Другие» 2000.
  `MIGRATION_12_13` заодно раскладывает по «Другим» все виды с
  `source != 'APP'`, добавленные до появления подборок, и создаёт саму строку
  только если такие виды есть — пустых пользовательских подборок в приложении
  не бывает (они удаляются вместе с последним ушедшим грибом).

  **Гейт `EnsureDefaultCollectionsUseCase` считает не `COUNT(*)`, а строки с
  `source = 'COUNTRY'`** — и это не косметика. Вторая половина гейта читается
  как «все N страновых пресетов на месте»; с пользовательскими строками в той
  же таблице десяток своих подборок перекрыл бы недостачу стран, и страховка
  молча перестала бы срабатывать.
...
**v12→v13 — аддитивные `collections.source` (`NOT NULL DEFAULT 'COUNTRY'`) и
`collections.name` (nullable)** плюс бэкфилл «Других», см. раздел про
`collections` выше и `.claude/plans/user-collections.md`. `DEFAULT` в
`ALTER TABLE` при отсутствии `@ColumnInfo(defaultValue = ...)` у сущности —
тот же приём, что уже отработан на `categories.source` в v5→v6: значение
нужно ровно на время `ALTER` (иначе `NOT NULL`-колонку некуда заполнить у
существующих строк), в схеме его нет, и валидация Room это переживает.
