...
/**
 * Приписка к имени файла снимка, называющая ПОКОЛЕНИЕ отрисовки — не только пропорцию, хотя
 * пропорция в неё и вписана для читаемости.
 *
 * Нужна затем, чтобы устаревший снимок узнавался по одному имени файла, без чтения самого файла:
 * снимок рисуется раз в жизни прогулки, и никакого другого признака, по которому отличить
 * нарисованное старым кодом, у него нет. [leshy.mushrooms.map.domain.usecase.
 * BackfillWalkThumbnailsUseCase] считает устаревшим всё, названное не так, и перерисовывает.
 *
 * Отсюда правило: **поменял что угодно в том, как снимок выглядит, — поменяй и эту строку.**
 * Не только пропорцию: обводку точек, толщину маршрута, цвета. Иначе снимки, нарисованные прежним
 * кодом, останутся лежать под именем нового поколения, и правку увидят одни лишь новые прогулки.
 *
 * История, ради которой это написано так подробно: первая редакция называла строку «геометрией» и
 * требовала менять её вместе с [WALK_THUMBNAIL_ASPECT_RATIO]. Уже в следующей правке — обводка
 * точек находок — менялась не геометрия, а рисунок, буквы `w16x9` остались бы верны, и снимки,
 * перерисованные днём раньше, обводки бы не получили никогда.
 *
 * Поколения: `_w16x9` — 16:9, точки без обводки; `v2` — с обводкой в 0.4 радиуса, слишком толстой;
 * `v3` — с обводкой в 0.15; `v4` — с полями вокруг маршрута, до него маршрут упирался в рамку
 * (на iOS полей не было вовсе, на Android они перестали быть пропорциональными размеру снимка).
 */
const val WALK_THUMBNAIL_VARIANT = "_w16x9v4"
