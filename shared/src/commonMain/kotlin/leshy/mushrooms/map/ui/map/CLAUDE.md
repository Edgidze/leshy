...
- **iOS: главный поток блокируется внутри `MapLibre.framework` на уходе в
  ...
  в `TH_WAIT` — та же картина (что-то высокоприоритетное крутится/ждёт
  вечно, пул MapLibre бездействует), что и в первом инциденте. Гипотеза
  «сеть пропала во время загрузки тайла»
