...
Внутри самого онбординга экранов теперь четыре (обзорная страница →
конфиденциальность → подборки, плюс выбор языка в стороне), и они тоже не маршруты —
это `when` по `OnboardingUiState.step` внутри одного composable, ровно по той
же причине. Шаги и их порядок — `presentation/CLAUDE.md`.
