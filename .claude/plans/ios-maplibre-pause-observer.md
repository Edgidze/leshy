...
1. **Спросить пользователя, воспроизводится ли.** Полосы на экране —
   симптом, который либо повторится на том же сценарии, либо был разовым
   отказом железа. Без этого выбор между «чинить GPU-путь» и «не трогать»
   делается монеткой.
2. **Ловить следующий раз с логом.** Панику мы прозевали; поставить
   `idevicesyslog` на запись в файл во время работы приложения — тогда у
   следующего инцидента будут последние секунды перед отказом.
3. **Найти источник 177 пробуждений/с.** Наших коротких таймеров нет, значит
...
4. **Найти, откуда 591 МБ пика.** Кандидаты: каталог на 408 видов с
   картинками, тайлы офлайн-участка, снапшоты карты.

Пункты 3 и 4 — это шаг 3 расследования, ставший измеримым. Пункт 2 —
