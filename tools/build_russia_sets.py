...
# Набор, в который попадает вид, отмеченный поиском по всему каталогу и не входящий больше никуда.
# Без этого такой вид жил в ленте «Записи», но ни в одной подборке экрана «Мои грибы» не
# показывался — «в ленте есть, в подборках нет» (замечание владельца 2026-09-26).
PICKED_FALLBACK_SET = "other"
...
DROPPED = {
    ...
    "suillus_luteus": "второй маслёнок: в «Сосновом бору» он стоял рядом с «Маслёнком» базы и означал"
                      " то же самое; оставлена широкая плитка `suillus_luteus__2` («Маслёнок»,"
                      " `Suillus sp.`). Сам ключ ещё и переименован в источнике — «Маслята» во"
                      " множественном числе выбивались из общего правила (`docs/catalog/edits.md`)",
    "collybia_nuda": "вторая рядовка фиолетовая: `Collybia nuda` и `Lepista nuda` — синонимы"
                     " одного вида, и русское имя у них одно и то же. Оставлен `lepista_nuda` —"
                     " он в базовом наборе, у него 26 языков против 10 и `importance` 5.0 против"
                     " 4.0; картинки у обоих один и тот же гриб. Дубль был помечен ещё разметкой"
                     " исследования («дубль/перекрытие с lepista_nuda», `species-ru-171-groups.csv`),"
                     " но в наборы всё равно попадал: пометка стояла в колонке `review`, а не в"
                     " `set_D`, которую читает этот скрипт",
    "cyclocybe_cylindracea": "второй опёнок тополиный (`cyclocybe_aegerita` и"
                             " `cyclocybe_cylindracea` — синонимы одного вида, в «Юге и Кавказе»"
                             " стояли двумя одинаковыми плитками); оставлен `aegerita` — у него на"
                             " картинке шляпки темнее (решение владельца)",
}
